...
# delete helm
for namespaces in kubesphere-system kubesphere-devops-system kubesphere-monitoring-system kubesphere-logging-system openpitrix-system kubesphere-monitoring-federated; do
...
if [[ $? -eq 0 ]]; then
	...
	for kubefed in $(kubectl api-resources --namespaced=true --api-group=types.kubefed.io -o name); do
		kubectl delete -n kube-federation-system $kubefed --all 2>/dev/null
	done
	for kubefed in $(kubectl api-resources --namespaced=false --api-group=types.kubefed.io -o name); do
		kubectl delete $kubefed --all 2>/dev/null
	done
	# delete kubefed core resouces
	for kubefed in $(kubectl api-resources --namespaced=true --api-group=core.kubefed.io -o name); do
		kubectl delete -n kube-federation-system $kubefed --all 2>/dev/null
	done
	for kubefed in $(kubectl api-resources --namespaced=false --api-group=core.kubefed.io -o name); do
		kubectl delete $kubefed --all 2>/dev/null
	done
...
delete_role_bindings() {
	for rolebinding in $(kubectl -n $1 get rolebindings -l iam.kubesphere.io/user-ref -o jsonpath="{.items[*].metadata.name}"); do
...
delete_roles() {
	...
	for role in $(kubectl -n $1 get roles -l iam.kubesphere.io/role-template -o jsonpath="{.items[*].metadata.name}"); do
...
delete_cluster_roles() {
	for role in $(kubectl get clusterrole -l iam.kubesphere.io/role-template -o jsonpath="{.items[*].metadata.name}"); do
		kubectl delete clusterrole $role 2>/dev/null
	done

	for role in $(kubectl get clusterroles | grep "kubesphere" | awk '{print $1}' | paste -sd " "); do
		kubectl delete clusterrole $role 2>/dev/null
	done
}
delete_cluster_roles
...
delete_cluster_role_bindings() {
	for rolebinding in $(kubectl get clusterrolebindings -l iam.kubesphere.io/role-template -o jsonpath="{.items[*].metadata.name}"); do
		kubectl delete clusterrolebindings $rolebinding 2>/dev/null
	done

	for rolebinding in $(kubectl get clusterrolebindings | grep "kubesphere" | awk '{print $1}' | paste -sd " "); do
		kubectl delete clusterrolebindings $rolebinding 2>/dev/null
	done
}
delete_cluster_role_bindings
...
# delete workspaces
for ws in $(kubectl get workspaces -o jsonpath="{.items[*].metadata.name}"); do
...
for webhook in ks-events-admission-validate users.iam.kubesphere.io network.kubesphere.io validating-webhook-configuration resourcesquotas.quota.kubesphere.io; do
	kubectl delete validatingwebhookconfigurations.admissionregistration.k8s.io $webhook 2>/dev/null
done

# delete mutatingwebhookconfigurations
for webhook in ks-events-admission-mutate logsidecar-injector-admission-mutate mutating-webhook-configuration; do
...
for workspacetemplate in $(kubectl get workspacetemplates.tenant.kubesphere.io -o jsonpath="{.items[*].metadata.name}"); do
...
# delete relevance ns
for ns in kube-federation-system kubesphere-alerting-system kubesphere-controls-system kubesphere-devops-system kubesphere-devops-worker kubesphere-logging-system kubesphere-monitoring-system kubesphere-monitoring-federated openpitrix-system kubesphere-system; do
